    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const getProfile = await profileRepository
                ...
                .leftJoinAndSelect("user.profile_image", "profile_image", "profile_image.is_active = :isActive", { isActive: true })
                .leftJoinAndSelect("user.cover_image", "cover_image", "cover_image.is_active = :isActive", { isActive: true })
                ...
                .where("user.id = :userId AND user.is_active = :isActive AND user.is_artist = :isArtist", { 
                    userId: userId, 
                    isActive: true,
                    isArtist: false
                })
...
userAuthRouter.patch(
    ...
    async (req: Request, res: Response) => {
        try {
            ...
            const imageIds = [];            
            if (profileDto.profile_image_id !== undefined) imageIds.push(profileDto.profile_image_id);
            if (profileDto.banner_image_id !== undefined) imageIds.push(profileDto.banner_image_id);
            if (profileDto.cover_image_id !== undefined) imageIds.push(profileDto.cover_image_id);
            ...
            // update user field
            if (profileDto.first_name !== undefined) user.first_name = profileDto.first_name;
            if (profileDto.last_name !== undefined) user.last_name = profileDto.last_name;
            if (profileDto.birth_date !== undefined) user.birth_date = new Date(profileDto.birth_date);
